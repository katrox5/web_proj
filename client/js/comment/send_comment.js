...
function add_to_db(){
	father_comment = target_comment_id;
	// alert(father_comment);
	...
	var pre_text = "";
	if(comment_id != target_comment_id){
		for (var i = 0; i < usernum; i++) {
			if (AllUsers.content[i][Idx_user_id_byAllUsers] == target_user_id) {
				pre_text = "回复" + "[" + AllUsers.content[i][Idx_nickname] + "]：";
			}
		}
	}
    ...
    // 通过验证，调用添加问题的函数
    add_text_request(useID,pre_text + text,father_comment);
